...
# Tcmb0			2.7255		The temperature of the CMB today in Kelvin.
...
class Cosmology():
	...
	def __init__(self, name = None, flat = True, \
		...
		self.H0 = H0
		self.h = H0 / 100.0
		self.Omh2  = self.Om0 * self.h * self.h
		self.Ombh2 = self.Ob0 * self.h * self.h 
		self.sigma8 = sigma8
		self.ns = ns
		self.Tcmb0 = Tcmb0
	...
	def integral(self, integrand, z_min, z_max):
		...
		min_is_array = isArray(z_min)
		max_is_array = isArray(z_max)
	...
	# Sound horizon in Mpc (not 1/h Mpc)
	# e.g., see eq 4-6 in Eisenstein & Hu 1998
	
	def SoundHorizon(self, z_min = 1200., z_max = numpy.inf):
		return 44.5*numpy.log(9.83/self.Omh2)/numpy.sqrt(1.0+10.*numpy.power(self.Ombh2,3./4.))
